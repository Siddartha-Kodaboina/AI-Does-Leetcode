...
# Function to generate realistic interview conversations
def generate_interview_conversation(question_content):
    prompt = f"""
    Simulate a detailed and realistic technical interview for the given problem statement:

    {question_content}

    The interview should be structured as follows:
    1. User1 (interviewer) should start by asking the initial question and guiding the conversation with clear, insightful follow-up questions.
    2. User2 (interviewee) should begin by presenting an initial naive solution and progressively move towards an optimal solution, discussing each step with clarity.
    3. User2 should ask relevant clarifying questions when needed, and User1 should provide appropriate hints and clarifications to steer the conversation.
    4. User2 should explain the data structures and algorithms considered at each step, with discussions on their strengths and weaknesses.
    5. The discussion should include detailed explanations of the time and space complexities for each approach proposed by User2.
    6. The conversation should showcase how a successful technical interview transitions from a basic understanding to an optimal solution, emphasizing improvements in time and space complexities.

    The conversation format should be:

    <conversation>
    <user1>User1 dialogue</user1>
    <user2>User2 dialogue</user2>
    <user1>User1 dialogue</user1>
    <user2>User2 dialogue</user2>
    ...
    </conversation>

    The content should reflect a professional, realistic interaction with technical depth and a clear progression from a naive solution to an optimal one.
    """
    
    # Call OpenAI's GPT model to generate the conversation
    chat = ChatOpenAI(api_key=OPENAI_API_KEY, model="gpt-4o")
    response = chat.invoke([HumanMessage(content=prompt)])
    conversation_text = response.content
    
    # Clean the response to match the desired format
    conversation_text = conversation_text.replace("<conversation>", "").replace("</conversation>", "").strip()
    return conversation_text


# Function to convert text to audio using Amazon Polly
def convert_text_to_audio(conversation_text, voice_ids=('Matthew', 'Joanna'), output_format='mp3'):
    dialogues = []
    segments = []
    current_voice = 0  # Toggle between voices for User1 and User2
    
    # Pre-process the conversation
    lines = conversation_text.splitlines()
    for line in lines:
        if line.startswith("<user1>"):
            dialogues.append((line.replace("<user1>", "").replace("</user1>", "").strip(), voice_ids[0]))
        elif line.startswith("<user2>"):
            dialogues.append((line.replace("<user2>", "").replace("</user2>", "").strip(), voice_ids[1]))

    # Generate audio for each dialogue part
    for dialogue, voice_id in dialogues:
        response = polly.synthesize_speech(Text=dialogue, OutputFormat=output_format, VoiceId=voice_id)
        if 'AudioStream' in response:
            temp_file = f"/tmp/{uuid.uuid4()}.mp3"
            with open(temp_file, 'wb') as file:
                file.write(response['AudioStream'].read())
            segments.append(AudioSegment.from_file(temp_file))
    
    # Combine all audio segments
    final_audio = sum(segments)
    output_path = f"/tmp/{uuid.uuid4()}_interview.mp3"
    final_audio.export(output_path, format=output_format)
    
    return output_path

# Function to upload audio to S3
def upload_audio_to_s3(file_path, question_id):
    s3_key = f"{question_id}/interview_audio.mp3"
    try:
